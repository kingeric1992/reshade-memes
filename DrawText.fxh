...
texture Texttex < source = "FontAtlas.png"; > { Width  = 512; Height = 512; };
sampler samplerText { Texture = Texttex; };



// draw text in relative coord
// size relative to uv scale.
#define DrawText_String( rect, arr, len, res ) { \
    float2 tuv = uv_pos / size; tuv.y = saturate(tuv.y), tuv.x *= 2.; \
    float  id = arr[tuv.x]; \
    res = tex2D(samplerText, ( frac(uv) + float2( id % 14.0, trunc(id / 14.0)) ) \
        / float2( _DRAWTEXT_GRID_X, _DRAWTEXT_GRID_Y) ).x * step(0,tuv.x) * step(tuv.x, len); \
}
